"""
    Filename: woolies.py
    Description: Contains all the functions that retrieve and process found products
"""
# Initialization
import time
import pandas as pd
        ...
        for category, keywords in category_dict.items():
            ...
            if key in keywords:
                categorized_items[category] = (
                    categorized_items.get(category, []) + value
                )
                break
    # Filter out ones with empty list
    categorized_items = {
        key: value for key, value in categorized_items.items() if value
    }
    # Remove duplicate
...
def find_product(
    ...
    df, 
    k: str, 
    filter_ingredient=True, 
    bad_list: List[str] = []
) -> pd.DataFrame:
    # Remove the words that are not needed
    all_replace = read_txt("data/all_replace.txt")
    for i in all_replace:
        if i in product:
            product = i

    words_to_remove = read_txt("data/remove_words.txt")
    ...
    words_to_pluralize = read_txt("data/words_pluralize.txt")
    exit_loop = False
    ...
    for keyword in product_split:
        selected_rows = selected_rows[
            selected_rows["Product Name"].str.contains(
                rf"\b{re.escape(keyword)}\b", case=False
            )
        ]
    ...
    if product == "butter":
        selected_rows = selected_rows[
            selected_rows["Sap Category Name"].str.lower()
            == "dairy - butter & margarine"
        ]
    if "spaghetti" in product:
        selected_rows = selected_rows[
            selected_rows["Sap Sub Category Name"].str.lower() == "pasta"
        ]
    if product == "ginger":
        selected_rows = selected_rows[
            selected_rows["Department"].str.lower() != "drink"
        ]
    if "basil" in product:
        selected_rows = selected_rows[
            selected_rows["Sap Category Name"].str.lower() != "CANNED VEGETABLES"
        ]
    ...
    if any(word in product for word in ["parmesan", "cheddar", "mozzarella", "cheese"]):
        selected_rows = selected_rows[
            selected_rows["Department"].str.lower() == "dairy"
        ]
    """"""
    ...
    clean_products_df = pd.DataFrame(
        columns=[
            "Product Name",
            "Ingredients",
            "Cup Price",
    ...
    for product_name, ingredients, cup_price, price, stockcode, image, cup in zip(
        product_names, ingredients_series, cup_prices, price, stockcode, image, cup
    ):
        clean = True
                ...
                for ingredient in ingredients_list:
                    ...
                    for bad_item in bad_list:
                        ...
                        all_words_present = all(
                            word in ingredient.lower() for word in bad_words
                        )
        ...
        emulsifier = sum(
            ingredient.lower().count("emulsifier") for ingredient in ingredients_list
        )
...
def get_all_product(
    ...
    for product in categorized_items.values():
        processed_product = p.singular_noun(product.lower()) or product.lower()

        # Renew - nick
        # for key in known_category.keys():
        #     if processed_product in known_category[key]:
        #         known_product[key].extend(product)
        #         categorized_items.pop(categorized_items[product])
        #         break
                

        for k, v in known_category.items():
            if processed_product in v:
                # known_product[k] = known_product.get(k) + [product]
                known_product[k].extend(product)
                categorized_items.pop(categorized_items[product])
    ...
    # Combine the known categories and the ones from ChatGPT
    categorized_items = {
        key: known_product[key] + categorized_items[key]
        for key in set(known_product) | set(categorized_items)
    }
    # Filter out ones with empty list
    categorized_items = {
        key: value for key, value in categorized_items.items() if value
                ...
                all_res[product].append(
                    {
                        ...
                        "cup_price": cup_price,
                        "price": price,
                        "stockcode": "https://www.woolworths.com.au/shop/productdetails/{}".format(
                            stockcode
                        ),
                        "image": image,
                        "cup": cup,
...
sample = pd.read_csv("data/Woolies bakery info.csv")
